...
def prepare_data(data):
    ...
    eye_left = eye_left.reshape(len(eye_left), img_size, img_size, 3)
    eye_right = eye_right.reshape(len(eye_right), img_size, img_size, 3)
    face = face.reshape(len(face), img_size, img_size, 3)
...
class EyeTracker(object):
    ...
    def train(self, train_data, val_data, lr=1e-3, batch_size=128, max_epoch=1000, min_delta=1e-4, patience=10, print_per_epoch=10, out_model='my_model',loadModel=None):
        ckpt = os.path.split(out_model)[1]
        ...
        with tf.compat.v1.Session() as sess:
            ...
            if(loadModel):
                ...
                saver.restore(sess, '/content/checkpoint')
...
def train(args):
    #print(args)
    org_train_data = load_data(args.input)
    rand_idx  = np.random.randint(0, len(org_train_data[0]), len(org_train_data[0]))
    org_train_data = [each[rand_idx] for each in org_train_data]
    train_size = int(0.7*len(org_train_data[0]))
    train_data = [each[:train_size] for each in org_train_data]
    val_data = [each[train_size:] for each in org_train_data]
    # train_size = 10
    ...
    train_loss_history, train_err_history, val_loss_history, val_err_history = et.train(train_data, val_data, \
                                            ...
                                            out_model=args.save_model,loadModel = args.load_model)
